...
func (mw loggingMiddleware) GetSecret(code string) (message string, err error) {
	defer func(begin time.Time) {
		mw.logger.Log(
			"method", "GetSecret",
			"code", code,
...
func (s *instrumentingService) GetSecret(code string) (message string, err error) {
	defer func(begin time.Time) {
		s.requestCount.With("method", "GetSecret").Add(1)
		s.requestLatency.With("method", "GetSecret").Observe(time.Since(begin).Seconds())
	}(time.Now())

	return s.Service.GetSecret(code)
}
