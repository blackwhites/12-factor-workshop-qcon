...
def pre_stop():
    ...
    if is_mongo_primary:
        ...
        try:
            log.info("stepping down as PRIMARY before shutting down %s", ip)
            ...
            try:
                # force
                log.info("[forcing] stepping down as PRIMARY before shutting down %s", ip)
        ...
        if wait_for_election:
            timeout = 0
            log.info("waiting for new primary to get elected")
            while True:
                ...
                if is_mongo_up(mongo_client, 1):
                    ...
                    if primary != None:
                        log.info('primary elected: {0!s}'.format(primary))
                        return True
...
def health():
    ...
    changed externally).
    """
    # TODO periodic mongodumps to an object store
